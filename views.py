...
def edit_loan_page_function(request, eid):
    ...
    if request.method == 'POST':
        # Replace these with the actual field names you want to check for duplicates
        account_number = request.POST.get('account_number')
        account_name = request.POST.get('account_name')
        ...
        loan_account.total_amount = loan_account.loan_amount + loan_account.interest_rate + loan_account.proccessing_fee
        loan_account.save()

        make_payment_amount = Decimal(request.POST.get('make_payment_amount', '0'))

        balance_amount, created = BalanceAmount.objects.get_or_create(loan_account=loan_account)
 
        balance_amount.balance_amount = loan_account.loan_amount - make_payment_amount
        balance_amount.save()
...
def loan_accounts (request):
  ...
  data4 = LoanAccounts.objects.filter(company=cmp)
  return render(request, 'company/loan_accounts.html',{'data':data,  'allmodules':allmodules,'staff':staff,'data2':data2,'data3':data3,'bank':bank,'data4':data4})
...
def ForId(request, id):
  ...
  cmp = company.objects.get(id=staff.company.id)
  bank = BankModel.objects.filter(company=cmp,user=cmp.user)
  data = LoanAccounts.objects.filter(id=id)
  data2 = makepayment.objects.filter(loan_account=id)
  data3 = AdditionalLOan.objects.filter(loan_account=id)
  data4 = LoanAccounts.objects.filter(company=cmp)
  data5 = LoanAccounts.objects.filter(id=id)
  return render(request, 'company/loan_accounts.html', { 'data5':data5, 'data': data,'data2':data2,'data3':data3,'data4':data4,'allmodules':allmodules,'staff':staff})
...
def additional_loan_function(request, eid):
    ...
    if request.method == 'POST':
        ...
        total_loan = Decimal(additional_loan)  
